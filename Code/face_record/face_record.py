...
detector = dlib.get_frontal_face_detector() #人臉辨識
sp = dlib.shape_predictor(predictor_path) #68點特徵點
...
while (True):
	...
	if not os.path.exists(directory):
		...
		if len(dir_list)>=max_user:
			...
			try:
				user_to_delete=int(input('Choose user (number) to delete: '))
...
while (True):
	...
	for i, d in enumerate(dets):
		...
		if len(idx)==1 and is_recording:
			shape = sp(frame, d)
			face_descriptor = np.array([facerec.compute_face_descriptor(frame, shape)]) #計算臉部128維的向量
			if len(temp_data)==0:
	...
	if cv2.waitKey(1) & 0xFF == ord('r') or len(temp_data) == 300:
		if is_recording==True or len(temp_data) == 300 :
			is_recording=False
			print (temp_data)
			print (len(temp_data))
